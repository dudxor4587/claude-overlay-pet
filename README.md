...
메이플 캐릭터 대신 직접 그린 에셋으로도 펫을 만들 수 있습니다. 우클릭 → **커스텀 펫 만들기…** 에서 상태별로 파일을 고릅니다 — **평소(idle)만 필수**, 나머지는 비워두면 평소 모습으로 재생됩니다.
...
- 파일은 GIF(애니메이션)·APNG·PNG. 투명 배경, 캐릭터는 오른쪽 보기 권장
- 긴 GIF는 16프레임으로 표본화, 큰 프레임은 384px로 축소
- 만든 뒤 우클릭 → **커스텀 펫 수정…** 으로 상태를 추가/교체할 수 있습니다

터미널에서는 폴더 규약으로도 됩니다 — 상태 이름 파일(`idle.gif`, `think.gif`, …)을 폴더에 넣고:

```sh
OverlayPet custom <GIF파일|에셋폴더> [--name 이름] [--no-use]
```
